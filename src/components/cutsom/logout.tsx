...
export default function Logout() {
  ...
  return (
    <>
      {showLightBox && (
        <Portal>
          <div className=" fixed h-screen top-0 left-0 right-0 bottom-0 z-10 bg-[rgba(255,255,255,0.5)] light-container  w-full flex items-center justify-center">
            {/* Log out pop up */}
            <div className="drop-shadow-lg  text-black bg-white p-8 rounded-2xl ">
              {/* Error message from the back end */}
              {error && <p className="text-center text-red-400 p-2 font-semibold">{error}</p>}
              <p className="capitalize text-lg font-medium mb-4 ">are sure you want to log out </p>
              <div className="buttons flex justify-between mx-6">
                <button onClick={handleLogout} className="bg-mainColor text-white rounded-full px-4 py-2 ">
                  Yes
                </button>
                <button onClick={hideLightBox} className="bg-mainColor text-white rounded-full px-4 py-2 ">
                  No
                </button>
              </div>
            </div>
